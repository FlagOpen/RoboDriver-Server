...
# ====================== 初始配置 ======================
echo "请注意！！！该安装程序为用户使用，适配 arm 架构，安装采集平台所需的软件、服务"
read -p "按回车键继续..."
...
# 检查目标目录是否已存在，不存在则拷贝，存在则跳过拷贝（仅更新权限）
...
for img in "${DOCKER_IMAGES[@]}"; do
    ...
    # 只询问是否从 Docker Hub 拉取镜像（ARM 架构版本）
    while true; do
        ...
        case "$pull_image" in
            y|Y)
                ...
                if sudo docker pull "liuyou1103/wanx-server-arm:latest"; then
                    echo "成功拉取镜像 liuyou1103/wanx-server-arm:latest"
                    # 重命名为本地需要的镜像名
...
# 启动后端服务
if [ -d "$BACKEND_DIR/arm" ]; then
    cd "$BACKEND_DIR/arm" || die "无法进入后端服务目录"
    echo "配置后台服务..."
    # 定义配置文件路径
    SETUP_FILE="$BACKEND_DIR/arm/setup.yaml"
