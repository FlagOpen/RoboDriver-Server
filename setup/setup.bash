...
if [ -f "$ROBOT_TYPE_FILE" ]; then
    ...
    echo -e "\n检测到上次使用的机器人类型: $LAST_ROBOT_TYPE"
    read -p "是否继续使用该类型？(y/n，默认y): " confirm
    if [[ -z "$confirm" || "$confirm" == "y" || "$confirm" == "Y" ]]; then
        robot_type="$LAST_ROBOT_TYPE"
        echo "已确认使用机器人类型: $robot_type"
    else
        # 重新输入机器人类型
        while true; do
            read -p "请输入新的机器人类型（例如：aloha, pika, realman 等）: " robot_type
    ...
    # 首次运行，输入机器人类型
    while true; do
        read -p "请输入您的机器人类型（例如：aloha, pika, realman 等）: " robot_type
        if [[ -z "$robot_type" ]]; then
            echo "错误：机器人类型不能为空，请重新输入！"
            continue
        fi
...
if [[ "$INSTALL_DOCKER" == "y" || "$INSTALL_DOCKER" == "Y" ]]; then
    ...
    echo "请注意！！！1. 您未安装 Docker，服务未通过容器启动，请手动部署运行环境；2. 请按版本自行安装机器控制程序后，再开始采集"
fi